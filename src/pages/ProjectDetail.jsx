...
import { PROJECT_DETAILS, PROJECTS, COMPANY } from '../data/site'
          ...
          <div>
            ...
            <Reveal delay={0.3}>
              <div className="grid grid-cols-2 gap-4 mt-8 pt-8 border-t border-soft">
                ...
                <KV label="Total Area" value={project.totalArea} />
                <KV label="RERA" value={project.rera?.number || 'On Request'} />
                <KV label="Price Range" value={project.priceRange} />
              </div>
            </Reveal>
          </div>
        </div>
      ...
      <section className="section-pad bg-page">
        <div className="container-x">
          <SectionHeading
            ...
            title={<>Built to the <span className="gold-text">finest standards</span></>}
            subtitle="Materials, fittings and finishes that reflect our commitment to lasting quality."
          />
          <div className="grid md:grid-cols-2 gap-6 mt-12">
            {project.specifications.map((s, i) => (
              <Reveal key={s.category} delay={i * 0.08}>
                <div className="card-glass p-7 h-full">
                  <h3 className="font-serif text-xl text-fg flex items-center gap-3">
            ...
            <Reveal delay={0.2}>
              <div className="mt-10 aspect-[4/3] overflow-hidden border border-soft">
                <iframe
                  ...
                  src={`https://www.google.com/maps?q=${encodeURIComponent(project.location + ', India')}&output=embed`}
      ...
      <section className="bg-page-alt border-y border-soft">
        <div className="container-x py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-gold-700 dark:text-gold-500 uppercase tracking-[0.25em] text-xs">RERA Disclosure</p>
      ...
      <div className="container-x py-20 md:py-28 relative grid lg:grid-cols-2 gap-12 items-start">
        ...
        <Reveal delay={0.15}>
          <form onSubmit={handleSubmit} className="bg-ink-800/60 border border-gold-500/15 p-7 backdrop-blur-sm space-y-5">
            ...
            <button type="submit" disabled={submitted} className="btn-gold w-full justify-center disabled:opacity-50">
              {submitted ? (<><CheckCircle2 className="w-4 h-4" /> Enquiry Received</>) : (<>Send Enquiry <Send className="w-4 h-4" /></>)}
            </button>
