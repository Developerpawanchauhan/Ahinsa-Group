...
export default function Contact() {
  ...
  return (
    <>
      ...
      {/* Contact info cards */}
      <section className="bg-ink-800 border-b border-gold-500/15">
          ...
          {[
            ...
            <Reveal key={c.title} delay={i * 0.1}>
              <div className="flex items-start gap-5">
                <div className="w-14 h-14 border border-gold-500/40 flex items-center justify-center text-gold-500 flex-shrink-0">
                  <c.icon className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-serif text-xl text-cream">{c.title}</h3>
                  <div className="text-ink-300 text-sm mt-2 space-y-1">
                    {c.lines.map((l, j) => (
                      <p key={j}>{l}</p>
                    ))}
                  </div>
            ...
            <Reveal>
              <form onSubmit={handleSubmit} className="mt-10 grid md:grid-cols-2 gap-5">
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs uppercase tracking-[0.2em] text-gold-500 mb-2">Message</label>
                  <textarea
                    name="message"
                    rows={5}
                    value={form.message}
                    ...
                    className="w-full bg-ink-800/60 border border-gold-500/20 px-5 py-4 text-cream placeholder:text-ink-400
                ...
                <div className="md:col-span-2">
                  ...
                  {submitted && (
                    <p className="text-gold-500 text-sm mt-4">
          ...
          <aside className="lg:col-span-2 space-y-6">
            <Reveal>
              <div className="card-glass p-7">
                <h3 className="font-serif text-2xl text-cream">Corporate Office</h3>
                ...
                <ul className="space-y-4 text-sm">
                  ...
                  <li className="flex gap-3 text-ink-200">
                    ...
                    <a href={`tel:${COMPANY.phone}`} className="hover:text-gold-500">{COMPANY.phone}</a>
                  </li>
                  <li className="flex gap-3 text-ink-200">
                    <Mail className="w-4 h-4 text-gold-500 flex-shrink-0 mt-0.5" />
                    <a href={`mailto:${COMPANY.email}`} className="hover:text-gold-500">{COMPANY.email}</a>
                  </li>
                </ul>
              </div>
            </Reveal>
            ...
            <Reveal delay={0.15}>
              <div className="card-glass p-7">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-serif text-xl text-cream">Office Hours</h3>
                </div>
            ...
            <Reveal delay={0.2}>
              <div className="card-glass p-7">
                <h3 className="font-serif text-xl text-cream">Site Visits</h3>
      ...
      <section className="bg-ink-800 border-t border-gold-500/15">
        <div className="container-x py-16">
          ...
          />
          <Reveal>
            <div className="mt-10 aspect-[16/9] md:aspect-[21/8] overflow-hidden border border-gold-500/20">
              <iframe
...
function Field({ name, label, type = 'text', value, onChange, required }) {
  return (
    <div>
      <label className="block text-xs uppercase tracking-[0.2em] text-gold-500 mb-2">{label}</label>
      <input
        type={type}
        name={name}
        value={value}
        onChange={onChange}
        required={required}
        className="w-full bg-ink-800/60 border border-gold-500/20 px-5 py-4 text-cream
                   placeholder:text-ink-400 focus:outline-none focus:border-gold-500 transition"
      />
    </div>
  )
}
